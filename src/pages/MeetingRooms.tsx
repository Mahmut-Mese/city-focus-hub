      ...
      <section className="section-padding bg-[#f5f0ea]">
        <div className="container-custom">
          <div className="mx-auto max-w-5xl">
            <div className="rounded-[36px] border border-black bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.09)] sm:p-8">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.16em] text-black/45">Live availability</p>
                  <h2 className="mt-3 font-sans text-5xl leading-none md:text-6xl">Check rooms and book instantly</h2>
                  <p className="mt-4 max-w-2xl text-lg text-black/55">
                    Guests can see real-time meeting room availability, pay online, and confirm the room immediately.
                  </p>
                </div>
              </div>
              ...
              {paymentDraft ? (
                <div className="mt-6">
                  <GuestPaymentPanel
                    ...
                    onConfirmPayment={handleConfirmGuestBookingPayment}
                    onCancelPayment={handleCancelGuestBookingPayment}
                  />
                </div>
              ) : null}
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_1fr_auto]">
                <div>
                  <Label htmlFor="meeting-room-start" className="mb-2 block text-sm text-black/55">Start</Label>
                  <Input
                    id="meeting-room-start"
                    type="datetime-local"
                    value={availabilityWindow.startAt}
                    onChange={(event) => handleAvailabilityWindowChange('startAt', event.target.value)}
                    className="h-12 rounded-2xl border-black/10 bg-[#fcfcfb]"
                  />
                </div>
                <div>
                  <Label htmlFor="meeting-room-end" className="mb-2 block text-sm text-black/55">End</Label>
                  <Input
                    id="meeting-room-end"
                    type="datetime-local"
                    value={availabilityWindow.endAt}
                    onChange={(event) => handleAvailabilityWindowChange('endAt', event.target.value)}
                    className="h-12 rounded-2xl border-black/10 bg-[#fcfcfb]"
                  />
                </div>
                <div className="flex items-end">
                  <Button
                    onClick={() => setAvailabilityWindow((current) => ({ ...current }))}
                    className="h-12 rounded-full bg-black px-6 text-sm font-medium text-white hover:bg-black/90"
                  >
                    Refresh
                  </Button>
                </div>
              ...
              <div className="mt-8 grid gap-5 lg:grid-cols-2">
                {isAvailabilityLoading ? (
                  <div className="col-span-full flex items-center gap-3 rounded-[24px] border border-black/10 bg-[#f9f7f3] p-6 text-sm text-black/50">
                    <LoaderCircle className="animate-spin" />
                    Loading meeting room availability...
                  </div>
                ) : null}

                {!isAvailabilityLoading && availabilityResources.map((resource) => (
                  <article key={resource.id} className="rounded-[28px] border border-black/10 bg-[#f9f7f3] p-6">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="text-sm uppercase tracking-[0.16em] text-black/40">{resource.type.replace('_', ' ')}</p>
                        <h3 className="mt-2 font-sans text-4xl leading-none">{resource.name}</h3>
                        <p className="mt-3 max-w-xl text-base text-black/55">{resource.description}</p>
                      </div>
                      <span className={`inline-flex rounded-full px-3 py-1.5 text-xs font-semibold ${resource.available ? 'bg-emerald-100 text-emerald-700' : 'bg-black/10 text-black/60'}`}>
                    ...
                    <div className="mt-6 grid gap-4 sm:grid-cols-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-black/40">Capacity</p>
                        <p className="mt-2 text-lg font-semibold text-black">{resource.capacity} people</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-black/40">Hourly rate</p>
                        <p className="mt-2 text-lg font-semibold text-black">{formatCurrency(resource.hourlyRateMinor)}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-black/40">Booking window</p>
                        <p className="mt-2 text-sm font-semibold text-black">
                          {formatDate(new Date(availabilityWindow.startAt).toISOString())} • {formatTime(new Date(availabilityWindow.startAt).toISOString())} - {formatTime(new Date(availabilityWindow.endAt).toISOString())}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
                      <p className="text-sm text-black/50">
                        Pay online as a guest and receive immediate booking confirmation.
                      </p>
                      <Button
                        onClick={() => openGuestBookingDialog(resource)}
                        disabled={!resource.available}
                        className="h-11 rounded-full bg-black px-5 text-sm font-medium text-white hover:bg-black/90 disabled:bg-black/20"
                      >
                        {resource.available ? 'Book and pay' : 'Not available'}
                      </Button>
                    </div>
                  </article>
                ))}
